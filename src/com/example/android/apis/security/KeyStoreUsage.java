...
public class KeyStoreUsage extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * Set up our {@code ListView} with an adapter that allows us to choose
		 * from the available entry aliases.
		 */
		...
		mGenerateButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				/*
				 * When the user presses the "Generate" button, we'll check the
				 * alias isn't blank here.
				 */
				...
				if (alias == null || alias.length() == 0) {
					...
					/*
					 * It's not blank, so disable the generate button while the
					 * generation of the key is happening. It will be enabled by
					 * the {@code AsyncTask} later after its work is done.
					 */
		...
		mPlainText.setOnFocusChangeListener(new OnFocusChangeListener() {
			...
			public void onFocusChange(View v, boolean hasFocus) {
				mPlainText.setTextColor(getResources().getColor(
	...
	private class AliasAdapter extends ArrayAdapter<String> {
		...
		/**
		 * This clears out all previous aliases and replaces it with the current
		 * entries.
		 */
		...
		protected Boolean doInBackground(String... params) {
			...
			try {
				...
				kpg.initialize(new KeyPairGeneratorSpec.Builder(
						getApplicationContext()).setAlias(alias)
						.setStartDate(now).setEndDate(end)
						.setSerialNumber(BigInteger.valueOf(1))
						.setSubject(new X500Principal("CN=test1")).build());
