...
public class DeviceAdminSample extends PreferenceActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 实例化DevicePolicyManager
		mDPM = (DevicePolicyManager) getSystemService(Context.DEVICE_POLICY_SERVICE);
		// 2.申请权限：DeviceAdminSampleReceiver 这个类是继承了DeviceAdminReceiver
		mDeviceAdminSample = new ComponentName(this,
				DeviceAdminSampleReceiver.class);
	}

	/**
	 * 为了能够显示出 headers 中的布局列表，需要在继承的 PreferenceActivity 类中实现 onBuildHeaders()
	 * 回调方法
	 */
	@Override
	public void onBuildHeaders(List<Header> target) {
		/**
		 * Settings中的每一个item都是一个header，
		 * 而你这些header是在对应device_admin_headers.xml文件中配置的
		 * 。通过loadHeadersFromResource (R.xml.device_admin_headers,
		 * target);将其加载进来。loadHeadersFromResource这个函数就是一个xml的文件的解析函数
		 * ，得到header并将其放入headers中。
		 */
	...
	public static class AdminSampleFragment extends PreferenceFragment
		...
		// Preferencescreen，它不仅可以作为设置界面显示，而且还能够启动activity
	 ...
	 */
	public static class GeneralFragment extends AdminSampleFragment implements
		...
		protected void reloadSummaries() {
			...
			String cameraSummary = getString(mDPM
					.getCameraDisabled(mDeviceAdminSample) ? R.string.camera_disabled
	 ...
	 */
	public static class QualityFragment extends AdminSampleFragment implements
			OnPreferenceChangeListener {
	...
	public static class ExpirationFragment extends AdminSampleFragment
		...
		private String getExpirationStatus() {
			...
			long localExpiration = mDPM
					.getPasswordExpiration(mDeviceAdminSample);
	 ...
	 */
	public static class LockWipeFragment extends AdminSampleFragment implements
	...
	public static class EncryptionFragment extends AdminSampleFragment
		...
		public void onResume() {
			...
			mRequireEncryption.setChecked(mDPM
					.getStorageEncryption(mDeviceAdminSample));
		}
