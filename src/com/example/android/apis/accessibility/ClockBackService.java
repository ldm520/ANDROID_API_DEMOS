 ...
 */
/**
 * ClockBackService——这项服务是基于最初AccessibilityService的实现，可以作为开发基本的无障碍服务的基础，
 * 这些服务都是兼容Android1.6(API级别4)以上版本。
 * 个人是同博文同步学习:http://blog.csdn.net/wyzxk888/article/details/9006881
 * @description：
 * @author ldm
 * @date 2016-5-6 上午9:52:19
 */
public class ClockBackService extends AccessibilityService {
	 ...
	 * <p>
	 * <strong> Note: This code sample will work only on devices shipped with
	 * the default Clock application. </strong>
	 * </p>
	 */
	// This works with AlarmClock and Clock whose package name changes in
	// different releases
	private static final String[] PACKAGE_NAMES = new String[] {
			"com.android.alarmclock", "com.google.android.deskclock",
	...
	static {
		sVibrationPatterns.put(AccessibilityEvent.TYPE_VIEW_CLICKED,
				new long[] { 0L, 100L });
		sVibrationPatterns.put(AccessibilityEvent.TYPE_VIEW_LONG_CLICKED,
				new long[] { 0L, 100L });
		sVibrationPatterns.put(AccessibilityEvent.TYPE_VIEW_SELECTED,
				new long[] { 0L, 15L, 10L, 15L });
		sVibrationPatterns.put(AccessibilityEvent.TYPE_VIEW_FOCUSED,
				new long[] { 0L, 15L, 10L, 15L });
		sVibrationPatterns.put(AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED,
				new long[] { 0L, 25L, 50L, 25L, 50L, 25L });
		sVibrationPatterns.put(AccessibilityEvent.TYPE_VIEW_HOVER_ENTER,
				new long[] { 0L, 15L, 10L, 15L, 15L, 10L });
		sVibrationPatterns.put(INDEX_SCREEN_ON, new long[] { 0L, 10L, 10L, 20L,
				20L, 30L });
		sVibrationPatterns.put(INDEX_SCREEN_OFF, new long[] { 0L, 30L, 20L,
				20L, 10L, 10L });
	}
				...
				mTts = new TextToSpeech(mContext,
						new TextToSpeech.OnInitListener() {
							public void onInit(int status) {
								// Register here since to add earcons the TTS
								// must be initialized and
								// the receiver is called immediately with the
								// current ringer mode.
	...
	private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (AudioManager.RINGER_MODE_CHANGED_ACTION.equals(action)) {
				...
				Log.w(LOG_TAG, "Registered for but not handling action "
						+ action);
			}
		}
		 ...
		 * 
		 * @param feedbackIndex
		 *            The index of the feedback in the statically mapped
		 *            feedback resources.
		 */
		private void provideScreenStateChangeFeedback(int feedbackIndex) {
			// We take a specific action depending on the feedback we currently
			// provide.
			...
			case AccessibilityServiceInfo.FEEDBACK_AUDIBLE:
				mHandler.obtainMessage(MESSAGE_PLAY_EARCON, feedbackIndex, 0)
						.sendToTarget();
				return;
			case AccessibilityServiceInfo.FEEDBACK_HAPTIC:
				mHandler.obtainMessage(MESSAGE_VIBRATE, feedbackIndex, 0)
						.sendToTarget();
				return;
			default:
	...
	public void onServiceConnected() {
		...
		// In Froyo the broadcast receiver for the ringer mode is called back
		// with the
		// current state upon registering but in Eclair this is not done so we
		// poll here.
	...
	private String generateScreenOnOrOffUtternace(int feedbackIndex) {
		...
		int maxRingerVolume = mAudioManager
	 ...
	 * Approach: Take over the haptic feedback by configuring this service to
	 * provide such and do so. This way the system will not call the default
	 * haptic feedback service KickBack.<br/>
	 * Take over the audible and spoken feedback by configuring this service to
	 * provide such feedback but not doing so. This way the system will not call
	 * the default spoken feedback service TalkBack and the default audible
	 * feedback service SoundBack.
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * 3. {@link AudioManager#RINGER_MODE_NORMAL} Goal: Provide custom spoken,
	 * default audible and default haptic feedback.<br/>
	...
	private void configureForRingerMode(int ringerMode) {
		if (ringerMode == AudioManager.RINGER_MODE_SILENT) {
			// When the ringer is silent we want to provide only haptic
			// feedback.
			mProvidedFeedbackType = AccessibilityServiceInfo.FEEDBACK_HAPTIC;

			// Take over the spoken and sound feedback so no such feedback is
			// provided.
			...
			mHandler.obtainMessage(MESSAGE_PLAY_EARCON, INDEX_RINGER_SILENT, 0)
					.sendToTarget();
			...
			// Use only an earcon to announce ringer state change.
			mHandler.obtainMessage(MESSAGE_PLAY_EARCON, INDEX_RINGER_VIBRATE, 0)
					.sendToTarget();
			...
			// Use only an earcon to announce ringer state change.
			mHandler.obtainMessage(MESSAGE_PLAY_EARCON, INDEX_RINGER_NORMAL, 0)
					.sendToTarget();
		}
	}
	...
	public void onAccessibilityEvent(AccessibilityEvent event) {
		...
		// Here we act according to the feedback type we are currently
		// providing.
		if (mProvidedFeedbackType == AccessibilityServiceInfo.FEEDBACK_SPOKEN) {
			mHandler.obtainMessage(MESSAGE_SPEAK, formatUtterance(event))
					.sendToTarget();
		} else if (mProvidedFeedbackType == AccessibilityServiceInfo.FEEDBACK_AUDIBLE) {
			mHandler.obtainMessage(MESSAGE_PLAY_EARCON, event.getEventType(), 0)
					.sendToTarget();
		} else if (mProvidedFeedbackType == AccessibilityServiceInfo.FEEDBACK_HAPTIC) {
			mHandler.obtainMessage(MESSAGE_VIBRATE, event.getEventType(), 0)
					.sendToTarget();
		} else {
			throw new IllegalStateException("Unexpected feedback type "
					+ mProvidedFeedbackType);
	...
	private String formatUtterance(AccessibilityEvent event) {
		...
		// There is no event text but we try to get the content description
		// which is
		// an optional attribute for describing a view (typically used with
		// ImageView).
	 ...
	 * 
	 * @param earconId
	 *            The id of the earcon to be played.
	 */
