	...
	public class MyAnimationView extends View implements
		...
		private void createAnimation() {
			if (animation == null) {
				 ...
				 * ofFloat()方法的第一个参数表示动画操作的对象（可以是任意对象），第二个参数表示操作对象的属性名字（
				 * 只要是对象有的属性都可以），第三个参数之后就是动画过渡值。
				 */
				ObjectAnimator yAnim = ObjectAnimator.ofFloat(ball, "y",
						ball.getY(), getHeight() - 50f).setDuration(1500);// y方向位移动画
				// 设置动画重复次数
				...
				ObjectAnimator xAnim = ObjectAnimator.ofFloat(ball, "x",
						ball.getX(), ball.getX() + 300).setDuration(1000);// x方向位移动画
				...
				ObjectAnimator alphaAnim = ObjectAnimator.ofFloat(ball,
						"alpha", 1f, .5f).setDuration(1000);// 透明度变化 动画
				AnimatorSet alphaSeq = new AnimatorSet();
				alphaSeq.play(alphaAnim);

				animation = new AnimatorSet();
				// 使用AnimatorSet类中的playTogether(Animator… items)方法实现多个动画同时执行的效果。
